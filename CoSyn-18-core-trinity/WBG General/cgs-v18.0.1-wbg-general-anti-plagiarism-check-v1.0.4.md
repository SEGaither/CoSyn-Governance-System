...
wbg_artifact:
  ...
  conflicts:
    []
  supersedes_artifact_id: "urn:cosyn:wbg:anti-plagiarism-check:1.0.3"
  supersedes_filename: "cgs-v17.0.0-wbg-general-anti-plagiarism-check-v1.0.3.md"
...
# WBG Anti-Plagiarism Check — v1.0.4
...
Registration must fail closed on incompatible CGS authority, duplicate capability ownership, integrity failure, or a conflicting higher-authority rule.
...
## 18. v1.0.4 Revision Note

v1.0.4 translation (WBG v18 modernization): removed `authority_ceiling: urn:cosyn:wbg:wbg-root-authority:1.1.0`, `minimum_wbg_root: "1.1.0"`, and `required_parent_cgs: "inherited_from_active_wbg_root"` from YAML. Replaced with `cgs_authority: "urn:cosyn:cgs:core-trinity:18.0.1"`. Section 15 (WBG Registration Requirements): removed `minimum WBG root` and `required parent CGS` bullet points (Root-authority coupling, per UR-2 resolution); translated "incompatible WBG Root" to "incompatible CGS authority". Bookfox path reference preserved as-is (per UR-3 resolution). All substantive check capability unchanged.
